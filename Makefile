...
retest: .cargo/config.toml ## force re-run tests
...
pc: staged verify  ## verify everything before commit

ci: verify pre-publish ## verify everything in a CI server

verify: fmt clippy test coverage-annotations doc outdated audit  ## verify regardless of context
